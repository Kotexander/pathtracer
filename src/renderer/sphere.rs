...
impl Sphere {
    ...
    pub fn intersect(&self, ray: &Ray, t_min: f32, t_max: f32) -> Option<Hit> {
        let pos = ray.pos - self.pos;

        let a = dot(&ray.dir, &ray.dir);
        let b = 2.0 * dot(&pos, &ray.dir);
        let c = dot(&pos, &pos) - (self.rad * self.rad);
...
mod tests {
    ...
    fn intersect() {
        ...
        let sphere = Sphere::new(0.5, Vector3::Z * 3.0);

        let hit = sphere.intersect(&ray, 0.0, f32::INFINITY).unwrap();
        assert_eq!(hit.t, 2.5);
        assert_eq!(hit.pos, Vector3::Z * 2.5);
        assert_eq!(hit.norm, -Vector3::Z);

        let ray = Ray::new(Vector3::ZERO, Vector3::Y);
        assert!(sphere.intersect(&ray, 0.0, f32::INFINITY).is_none());
    }
}
